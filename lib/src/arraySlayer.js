...
import { PRIMITIVE_TYPES, ISNAN, isObject, checkLengths, IterateeError } from "../utils/helpers.js";
...
class ArraySlayer {
  ...
  max(...arrays) {
    return !!arrays ? Math.max(...this.arr, ...arrays.flat(Infinity)) : Math.max(...this.arr);
  }

  min(...arrays) {
    return !!arrays ? Math.min(...this.arr, ...arrays.flat(Infinity)) : Math.min(...this.arr);
  }
  ...
  zip(...arrays) {
    this.arr = this.arr.map((item, idx) => arrays.reduce((a, b) => [...a, b[idx]], [item]));
    return this;
  }
  unzip(){
