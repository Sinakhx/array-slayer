...
export const hasNumber = (arr) => arr.some((item) => typeof item === "number");
...
export const hasAll = (primitives = [], arr) => {
    const set = new Set(arr);
    return primitives.every(primitive => set.has(primitive));
};

export const hasAllTypes = (types = [], arr) => {
    const set = new Set(arr);
